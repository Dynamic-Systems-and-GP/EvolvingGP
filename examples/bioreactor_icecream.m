    ...
    function [r]=getrefsignal(self,reftype)
        ...
        %type twotrains
            case 'twotrain'
                ...
                r21=repmat([r1-doff+dr; r1-doff-dr],[2,1])+self.r0;
                r22=repmat([r1+doff-dr; r1+doff+dr],[2,1])+self.r0;
                r2=[r21;r22];
        ...
        function init(self,varargin)
			...
			self.P.X_0		=0.0;		% biomass in the influent [g_VSS l^-1]
% 			self.P.S_0		=4.94  ;	% substrate in the influent [g_COD l^-1]
			self.P.Q		=1.49  ;    % substrate in the influent [l/day]
        ...
        function [y]=gety(self)
            self.bioreactorSim();
            y = self.getyr()+randn*self.noisestd;
            self.y=y;
        end
		...
		function bioreactorSim(self)
            ...
            s0ode=[self.S self.X]';
            [t,s1ode]=ode45(f,[0:self.Ts/10:self.Ts],s0ode); %11 steps per each run is minimum
            self.S   =s1ode(end,1);
            self.X   =s1ode(end,2);
		end
        ...
        function self=bioreactor_icecream(varargin)
            ...
            usat=struct('umin',1.5,'umax',10,'ymin',0.85,'ymax',1.35);
            normdata=struct('umean',5.75,'udelta',1,...
                            'ymean',1.1,'ydelta',0.5);
            if nargin>=2
                usat=varargin{2};
            end
            if nargin==3
